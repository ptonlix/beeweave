"""Batch planner for parallel beeweave-ingest subagent dispatch.
...
The agent calls `bwe batch-plan <vault> <source-dir> [options]`
...

{
  ...
  "merge_hint": "Run /beeweave-ingest on each batch in parallel, then run /beeweave-cross-linker once all batches are done."
}

The skill dispatches each batch as a parallel subagent call, then runs
beeweave-cross-linker once all agents report completion.
"""
...
def discover_sources(
    ...
    *,
    ...
    because beeweave-ingest Step 1c handles them via ast-extract separately.
    """
    files = []
    for dirpath, dirnames, filenames in os.walk(source_dir):
        current = Path(dirpath)
        try:
            relative_parts = current.relative_to(source_dir).parts
        except ValueError:
...
def _filter_assets_for_unchanged_articles(
    files: list[dict],
    vault: Path,
) -> tuple[list[dict], int]:
    """Skip article assets when their sibling index.md is already unchanged.

    Workbench articles are stored as ``<article>/index.md`` plus ``assets/``.
    The manifest tracks the markdown source, so a directory scan would otherwise
    keep re-planning unchanged images forever.
    """
    try:
        from beeweave.cache import check_sources
        ...
        unchanged_dirs = {str(Path(p).parent) for p in result["unchanged"]}
        filtered = []
        skipped = 0
        for f in files:
            path = Path(f["path"])
...
def plan_batches(
    ...
    *,
    ...
    if skip_unchanged and vault.is_dir():
        ...
        to_ingest, skipped_assets = _filter_assets_for_unchanged_articles(to_ingest, vault)
        skipped_unchanged += skipped_assets
    ...
    merge_hint = (
        ...
        "Once all batches complete, run /beeweave-cross-linker to wire up cross-references."
    )
