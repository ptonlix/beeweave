"""Content-hash cache for beeweave-ingest source tracking.

Provides a reliable, platform-independent alternative to running `sha256sum`
in the skill. The agent calls `bwe cache-check` / `cache-update`
...
def _manifest_hash(raw_hash: str) -> str:
    """Return the canonical manifest form for a SHA-256 hex digest."""
    return raw_hash if raw_hash.startswith("sha256:") else f"sha256:{raw_hash}"
...
def check_sources(vault: Path, source_paths: list[Path]) -> CheckResult:
    ...
    for path in source_paths:
        ...
        elif entry.get("content_hash") != _manifest_hash(current_hash):
...
def update_source(
    ...
    *,
    ...
    entry["content_hash"] = _manifest_hash(current_hash)
