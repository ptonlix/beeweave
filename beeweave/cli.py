...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if project_dir is not None:
        ...
        print("   2. Start your knowledge flywheel:")
        print("\n      1. Capture knowledge")
        print("         /beeweave-ingest <URL, file, or directory>")
        print("\n      2. Create · Writing flywheel")
        print("         /beeweave-url-capture <URL>")
        print("         /beeweave-article-writer Write an article from the captured source and my vault")
        print("         /beeweave-article-publisher <draft-path>")
        print("\n      Capture → connect → create → publish → distill → create again")
    else:
        print("   1. Open any project in your agent")
        print("   2. Use the portable knowledge skills installed for cross-project work:")
        print("      /beeweave-ingest /path/to/source")
        print("      /beeweave-update")
        print("      /beeweave-query what do I know about ...")
    if project_dir is not None:
        print("\n Knowledge-only workflows from any project:")
        print("   /beeweave-ingest    → add files, folders, URLs, or inbox content to your vault")
        print("   /beeweave-update    → sync durable project knowledge to your vault")
        print("   /beeweave-query     → query existing knowledge in your vault")
    print()
