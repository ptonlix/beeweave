...
Optional: install `beeweave[graph]` for the real Leiden algorithm via
...
def parse_vault_graph(vault: Path) -> tuple[dict[str, list[str]], dict[str, list[str]]]:
    ...
    skip_dirs = {"_meta", "_raw", "_archived", "_staging", "_archives", ".obsidian"}
    ...
    for page in pages:
        ...
        # Markdown links (when BEEWEAVE_LINK_FORMAT=markdown)
