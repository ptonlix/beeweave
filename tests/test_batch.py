...
class TestDiscoverSources:
    ...
    def test_skips_vault_meta_dir(self, vault):
        _write(vault / "_meta" / "taxonomy.md")
        _write(vault / "concepts" / "topic.md")
        files = discover_sources(vault, vault=vault)
        rel_paths = [Path(f["path"]).relative_to(vault).parts for f in files]
...
class TestPlanBatches:
    ...
    def test_skips_assets_when_article_index_unchanged(self, source_dir, vault):
        index = _write(source_dir / "article" / "index.md", "# Article")
        ...
        assert result["stats"]["skipped_unchanged"] == 3
        assert result["stats"]["to_ingest"] == 0
