...
BeeWeave 是一个 Agent 原生的创作工作台，用来围绕创作过程构建数据飞轮：获取素材，用 Agent 辅助创作，把关键内容沉淀成长期知识，复用这些上下文，再指导下一轮更好的素材获取。
...
公开 wiki 地址是 <https://ptonlix.github.io/beeweave/>。GitHub Pages 应使用
`gh-pages` 分支和 `/root` 作为发布源。
