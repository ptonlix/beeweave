...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  try {
    ...
    if (!rawDirectoryHandle || !(await hasWritePermission(rawDirectoryHandle))) {
      await showNeedsPopup("Open BeeWeave Capture and choose workbench/inbox/web first.");
      return;
    }
...
function createContextMenus() {
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({
      id: MENU_PAGE,
      title: "Capture page to workbench inbox",
      contexts: ["page"]
    });

    chrome.contextMenus.create({
      id: MENU_SELECTION,
      title: "Capture selection to workbench inbox",
      contexts: ["selection"]
    });
  });
}
