...
const INBOX_HANDLE_KEY = "workbench-inbox-folder";
...
const copyPathButton = document.querySelector("#copyPath");
const noteField = document.querySelector("#note");
const statusLine = document.querySelector("#status");
const inboxPathCommand = "awk -F= '/^BEEWEAVE_REPO=/{gsub(/^[ \\t\\\"]+|[ \\t\\\"]+$/, \"\", $2); print $2 \"/workbench/inbox/web\"; exit}' \"$HOME/.beeweave/config\"";

let rawDirectoryHandle;

pathCommand.textContent = inboxPathCommand;
init();
...
chooseFolder.addEventListener("click", async () => {
  ...
  try {
    ...
    setStatus("Selected. Captures will now write markdown files into workbench/inbox/web.", "success");
  } catch (error) {
...
copyPathButton.addEventListener("click", async () => {
  try {
    await navigator.clipboard.writeText(inboxPathCommand);
...
captureButton.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!rawDirectoryHandle) {
      setStatus("Choose workbench/inbox/web first.", "error");
      return;
    }
...
async function init() {
  ...
  setFolderStatus("No workbench/inbox/web folder selected");
}
...
async function loadRawHandle() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = transaction.objectStore(STORE_NAME).get(INBOX_HANDLE_KEY);
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error);
...
async function saveRawHandle(handle) {
  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.objectStore(STORE_NAME).put(handle, INBOX_HANDLE_KEY);
