---
name: beeweave-article-writer
description: |
  通用中文长文写作 skill。当用户需要撰写公众号文章、博客长文、深度文章、观点稿、产品体验稿、方法论文章、续写文章、根据素材产出长文时使用。触发词包括但不限于：写文章、写稿子、帮我写、续写、扩写、公众号文章、博客文章、长文、出稿、按我的风格写。也适用于用户丢过来一个 PDF、brief、新闻链接、语音转文字或任何素材说"帮我写篇文章"的场景。不要用于短内容、推特 thread、朋友圈短文或纯标题摘要生成。
---
...
同一篇长文默认只有一个当前工作稿，也就是 `draft_path`。用户要求继续改、润色、换开头、缩短、扩写、按反馈改时，优先更新这个工作稿并维护 frontmatter 的 `updated`，不要默认在 `articles/drafts/` 里制造多个完整版本文件。版本历史写入 trace。

每次保存或改写长文时，在 `$BEEWEAVE_WORKBENCH_PATH/writing/traces/` 下创建或更新 trace bundle：

```text
writing/traces/YYYY-MM-DD-article-<topic-slug>/
├── trace.md
└── trace.json
```
...
- `skill_name: beeweave-article-writer`
...
trace 默认只记录路径、摘要、diff summary 和学习状态，不复制完整草稿正文。只有用户明确要求“保存完整过程快照”“保留这个版本”“把初稿全文纳入 trace”或把某版标记为学习样本时，才在 trace bundle 的 `snapshots/` 子目录保存完整正文，并在 trace 里标记 `user_requested_full_snapshot: true` 或 `learning_sample: true`。这些快照不能写到 `articles/drafts/` 当作默认版本文件。

如果 trace 目录或 trace 文件写入失败，但长文草稿已经成功保存，必须把草稿路径交付给用户，并清楚说明 trace 记录失败。只有无法定位 Workbench 或无法创建 drafts 目录时，才按上面的工作区定位规则停止。
...
高质量长文基本可以归为五种原型，写之前先判断属于哪种，每种的写法重心不同：
...
**亲自下场**：这是高质量创作的核心原则。不是隔空评论，而是真的去做那件事。写的时候，要让读者感觉到"这个人真的做了这件事"，而不是"这个人在想象做这件事"。
...
**阅读分层与小标题**：长文必须让读者可以先扫结构再决定细读。正文在一级标题后，按内容层次添加 `##` 二级小标题，每个小标题承接一个明确阅读任务，比如痛点、核心机制、差异、场景、快速开始、适合人群、贡献方式、最终判断。小标题要短、具体、像真人写给读者看的路标，不要写成报告式标签。标题负责分层，段落内部仍然保持口语化、短段落、转场句和扣主线句。
...
**英雄之旅叙事弧**：很多好莱坞电影的底层叙事结构是英雄之旅，一个普通人被召唤去冒险，经历考验，获得宝物，带着变化回到日常。调查实验型和产品体验型文章也可以使用类似结构，先说遇到了什么问题或好奇心，再说怎么一步步去做、踩了什么坑，最后秀出那个让人「卧槽」的结果。读者跟着走完这个弧线，会有一种「我也跟着经历了一遍」的参与感，而不是被动接收信息。写的时候要注意，冒险的起点必须是一个具体的、读者能代入的困境或好奇（「我就想看看9.9的DeepSeek到底是什么」），而不是一个抽象的命题。何同学的很多视频也是英雄之旅的节奏，这个结构之所以好使，是因为它跟人类天然的叙事本能对齐，听故事的人会自动代入主角的位置。
...
2. **过度结构化**：不用bullet point罗列观点，不大量加粗，不把文章写成咨询报告或产品文档。长文需要 `##` 二级小标题做阅读分层，但小标题只是路标，不是报告大纲。不要使用"第一部分""第二部分""背景介绍""总结展望"这类机械标题；不要用三级标题层层嵌套；不要让标题替代自然转场。标题之间的正文仍然要靠口语化转场句（"说到这个""回到xxx这块""顺着上面的再聊聊"）和扣主线句推进。
3. **标点禁令**：
   - 不使用冒号"："，用逗号代替
   - 不使用破折号"——"
...
下面是一批适合中文长文的口语化表达，这些词组自带"活人感"，写作时可以主动、自然地使用：
...
【收尾】几种常见收法，选最合适的一种：
  ...
  ↓
【可选尾部】
如用户提供署名、引导语或固定结尾模板，则按用户要求输出。
如用户没有提供，不要自动添加固定署名、联系方式或账号口播。
```
...
- 需要插图的位置标注"图片"即可
- **必须有二级小标题**。一级标题 `#` 后的正文要用 `##` 切出清晰阅读层级，通常 5-10 个板块。每个板块标题回答读者正在看什么，标题要短、具体、有口语感，例如 `## 一个让我反复崩溃的问题`、`## 它和普通 LLM Wiki 的区别`、`## 快速开始`。不要为了形式硬凑标题，短文或用户明确要求无标题时可以例外，但公众号长文、博客长文、深度文章默认都要分层。
- 小标题不能破坏活人感。标题下面的正文仍然使用短段落、口语化转场、疑问句、断裂句和扣主线句；不要因为有标题就变成条目说明书。
...
这一层检查文章是否符合自然、具体、有判断力的中文长文模式，类似代码的单元测试。每一项都给出"是/否"判断。
...
- 有没有用疑问句来制造节奏的刹车和转向？
- 是否使用了必要的 `##` 二级小标题做阅读分层？（长文通常 5-10 个板块，标题短、具体、有口语感）
- 小标题之间是否仍然靠口语化转场和扣主线句自然推进？（不能变成报告式条目堆砌）
...
**修复方式**：这一层没有机械的修复方法。核心操作是：把"AI味重"的段落找出来，想象一个有真实经验、有明确判断的作者会怎么说这段话，然后用更口语、更私人、更不完美的方式重写。
