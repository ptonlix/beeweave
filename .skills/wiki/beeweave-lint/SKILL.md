...
**Before scanning anything:** follow the Retrieval Primitives table in `beeweave-core/SKILL.md`. Prefer frontmatter-scoped greps and section-anchored reads over full-page reads. On a large vault, blindly reading every page to lint it is exactly what this framework is built to avoid.
...
Every page *should* have a `summary:` frontmatter field — 1–2 sentences, ≤200 chars. This is what cheap retrieval (e.g. `beeweave-query`'s index-only mode) reads to avoid opening page bodies.
...
Check whether pages are being honest about how much of their content is inferred vs extracted. See the Provenance Markers section in `beeweave-core` for the convention.
...
**How to fix:**
- Run the `beeweave-cross-linker` skill targeted at the fragmented tag — it will surface and insert the missing links
...
**How to check:**
- Glob `$BEEWEAVE_VAULT_PATH/misc/*.md`
- For each page, read the `affinity` frontmatter field
- Flag pages where any single project's score ≥ 3

**How to fix:**
- Run the `beeweave-cross-linker` skill first if affinity scores look stale (e.g., `affinity: {}` on a page with many wikilinks)
...
Enforces the confidence + lifecycle frontmatter schema (see `beeweave-core/SKILL.md`, Confidence and Lifecycle section).
...
**How to check:** For pages that have both `base_confidence:` and `sources:` in frontmatter, recompute `base_confidence` using the formula in `beeweave-core/SKILL.md`. If the stored value differs from the recomputed value by more than 0.05, flag it as drift.
...
- For each pair, run a quick grep to count pages that link to both:
  ```bash
  grep -rl "\[\[ConceptA\]\]" "$BEEWEAVE_VAULT_PATH" --include="*.md" > /tmp/a.txt
  grep -rl "\[\[ConceptB\]\]" "$BEEWEAVE_VAULT_PATH" --include="*.md" > /tmp/b.txt
  comm -12 <(sort /tmp/a.txt) <(sort /tmp/b.txt) | wc -l
  ```
- Flag pairs with co-occurrence ≥ 3 that have no existing synthesis page

**How to fix:**
- Run `/beeweave-synthesize` to automatically discover and fill the top gaps
...
|---|---|---|
| `misc/web-martinfowler-articles-microservices.md` | `beeweave` | 4 |
...
|---|---|---|
| [[Caching]] × [[Consistency]] | 5 pages | Run `/beeweave-synthesize` |
| [[Testing]] × [[Observability]] | 3 pages | Run `/beeweave-synthesize` |
```
...
Triggered by `beeweave-lint --consolidate`. Switches from report-only to **act-and-report** — the "dream cycle" that runs periodically so the wiki self-heals.
...
5. Never merge pages — use `beeweave-dedup` for that. Only link, promote, demote, and flag.
...
- This is scoped to orphans only (different from `beeweave-cross-linker` which runs broadly).
...
- **Promote `draft` → `reviewed`:** pages where `lifecycle: draft` AND `created` > 30 days ago AND `base_confidence > 0.7`. Set `lifecycle: reviewed`, `lifecycle_changed: <today>`, `lifecycle_reason: "auto-promoted by beeweave-lint --consolidate: age>30d, confidence>0.7"`.
...
Read `_meta/taxonomy.md` for the alias mapping (e.g., `ml → machine-learning`). For each page, replace known alias tags with their canonical form in the `tags:` frontmatter field. This is a subset of `beeweave-tag-taxonomy`'s work — only alias fixes, no full audit.
...
sources: []
summary: Auto-generated consolidation report from beeweave-lint --consolidate run on <date>.
lifecycle: draft
...
```
beeweave-lint --consolidate — Dry Run
