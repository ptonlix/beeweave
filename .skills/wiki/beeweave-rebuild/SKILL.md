...
1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and optional QMD settings such as `QMD_WIKI_COLLECTION`
...
Archives live at `$BEEWEAVE_VAULT_PATH/_archives/`. Each archive is a timestamped directory containing a full copy of the wiki state at that point.
...
1. `beeweave-status` — to see all sources as "new"
2. `beeweave-claude-ingest` — to reprocess Claude history
3. `beeweave-codex-ingest` — to reprocess Codex session history
4. `beeweave-ingest` — to reprocess documents and any other raw data
...
Refresh QMD after restore (see "QMD Refresh After Live Wiki Changes"), then tell the user what was restored and suggest running `beeweave-lint` to check for any issues with the restored state.
