...
description: >
  ...
  report", "what's new in my wiki", "/beeweave-digest [period]", "summarize my recent learning", or
  wants a readable overview of recent wiki activity. Distinct from beeweave-status (which reports
  ingestion delta of sources) — beeweave-digest summarizes *knowledge*, not sources.
---
...
1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and `BEEWEAVE_LINK_FORMAT`.
...
3. Read `$BEEWEAVE_VAULT_PATH/log.md` — last 200 lines — for entries within the period (timestamps are ISO-8601 prefixed lines).
4. Read `$BEEWEAVE_VAULT_PATH/hot.md` for current session context.
5. If `$BEEWEAVE_VAULT_PATH/_insights.md` exists, read its **Anchor Pages** table — you'll use it later to identify which new pages became hubs.

## Step 1: Collect Pages Active in the Period

Glob all `.md` files under `$BEEWEAVE_VAULT_PATH`. Skip special/system files:
- `index.md`, `log.md`, `hot.md`, `AGENTS.md`, `_insights.md`
- Anything under `_meta/`, `_archives/`, `_staging/`
...
Also read `$BEEWEAVE_VAULT_PATH/_meta/taxonomy.md` (if it exists). Flag any tag from step 1 that **does not appear** in the taxonomy — these are new vocabulary words that emerged this period.
...
Scan active pages and `workbench/inbox/` for unresolved work:
...
Apply the link format from `beeweave-core/SKILL.md` (Link Format section) using `BEEWEAVE_LINK_FORMAT`. Default is `[[wikilink]]`.
...
- **Ambiguous claims**: [N] `^[ambiguous]` markers across [M] pages — run `/beeweave-synthesize` to resolve
- **Inbox pending**: [N] files in `workbench/inbox/` — run `/beeweave-ingest workbench/inbox` to promote them
- **Taxonomy gaps**: Tags `#newtag1`, `#newtag2` used but not in taxonomy — run `/beeweave-tag-taxonomy`
...
---
*Generated by beeweave-digest · [TIMESTAMP] · [N pages scanned in [VAULT_PATH]]*
```
...
*"Want me to save this as `workbench/articles/drafts/digest-YYYY-MM-DD.md`?"*

**If user prefixed with "save" or "write"** (e.g., `/beeweave-digest save` or "generate and save my weekly digest"):
- Write to `workbench/articles/drafts/digest-YYYY-MM-DD.md` (weekly/monthly) or `workbench/articles/drafts/digest-YYYY-MM-DD-daily.md` (daily)
- Add frontmatter:
  ```yaml
  ---
...
- **This skill only reads vault pages.** The only writes are the optional workbench draft, and the `log.md` append. It does not modify existing wiki pages.
- **Don't duplicate beeweave-status.** If the user asks "what needs ingesting" or "what's the delta", route to `beeweave-status`. This skill answers "what did I learn", not "what's pending".
