...
This skill can be invoked directly or via the `beeweave-history-ingest` router (`/beeweave-history-ingest pi`).

## Before You Start

1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and `PI_HISTORY_PATH` (defaults to `~/.pi/agent/sessions`)
...
- Add provenance markers using the convention in `beeweave-core`:
...
**Mark provenance** per the convention in `beeweave-core`:
...
**`hot.md`** — Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from the template in `beeweave-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Ingested 12 Pi sessions across 3 projects; surfaced patterns in CLI tooling and API design." Keep the last 3 operations. Update `updated` timestamp.
