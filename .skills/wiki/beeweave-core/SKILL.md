...
Quick captures and web clippings live in `workbench/inbox/`, not inside the vault. Files there are pending inputs rather than Layer 1 source libraries, but `beeweave-ingest` still moves them to `workbench/inbox/archived/` after promotion because some captures have no other copy.
...
The wiki lives at the path configured via `BEEWEAVE_VAULT_PATH` in `.env`.
...
### `.manifest.json`
Tracks every source file that has been ingested — path, timestamps, what wiki pages it produced. This is the backbone of the delta system. See the `beeweave-status` skill for the full schema.
...
visual (see *Academic papers* in `beeweave-ingest` for the PyMuPDF extraction recipe).
...
These are best-effort numbers written by the ingest skill at create/update time. `beeweave-lint` recomputes them and flags drift. The block is optional — pages without it are treated as fully extracted by convention.
...
- `target` — a wikilink (using the same format as `BEEWEAVE_LINK_FORMAT`) to the related page
...
- **Optional field** — omit the block entirely if no typed relationships are known. Untagged wikilinks remain valid and are treated as `related_to` by `beeweave-export`.
...
Skills that read `relationships:`: `beeweave-export` (emits typed edges), `beeweave-cross-linker` (writes typed entries when inferring links), `beeweave-query` (surfaces type in answers and walks the typed-edge graph for multi-hop "how is X connected to Y" path queries — bounded BFS over the `relationships:` adjacency, frontmatter-only).
...
# lifecycle_reason: "..."      # optional free-text — why the state changed; surfaced by beeweave-query
...
|---|---|---|
| `beeweave-ingest` (URL) | `0.17 + 0.5 × classify(url)` | `draft` |
| `beeweave-ingest` (single doc) | per-source classifier | `draft` |
| `beeweave-ingest` (multi-doc) | `min(N/3,1)×0.5 + avg_q×0.5` | `draft` |
| `beeweave-research` | varies, often 0.85+ | `draft` |
| `beeweave-capture` | 0.42 | `draft` |
| `*-history-ingest` | 0.42 | `draft` |
| `beeweave-update` | 0.59 | `draft` |
| `beeweave-synthesize` | `min(input_pages.base_confidence)` | `draft` |
...
- **Promote to `core`:** when a page accumulates ≥5 incoming wikilinks **or** is flagged as a bridge by `beeweave-status` insights mode
...
- `beeweave-ingest` reads `tier:` to decide whether to update a page on the current pass
- `beeweave-query` uses `tier:` to order candidates in the index pass and trim to context budget
- `beeweave-status` insights mode computes graph metrics and **suggests** tier assignments — it never writes them automatically
- `beeweave-lint` flags missing `tier:` on newly created pages (Phase 2 enforcement, same timeline as `base_confidence`)
...
Skills that consume this table: `beeweave-query`, `beeweave-cross-linker`, `beeweave-lint`, `beeweave-status` (insights mode). Any new skill that reads the vault should cite this section rather than reinvent the pattern.
...
All internal links connecting wiki pages are controlled by `BEEWEAVE_LINK_FORMAT` from the resolved config (default: `wikilink`).
...
When `BEEWEAVE_LINK_FORMAT=markdown`:
...
**Scope:** this setting affects only newly written or updated links. Existing vault content is never automatically migrated — users who want to convert old links can run the `beeweave-cross-linker` or `beeweave-lint` skill.

Every write skill reads `BEEWEAVE_LINK_FORMAT` from config before generating links and applies the correct format.

## Config Resolution Protocol

**All skills must resolve config using this algorithm — do not hard-code `.env` or `~/.beeweave/config` directly.** This ensures single-vault, multi-vault, project-local, and VPS setups all work correctly.

### Resolution order

0. **Inline profile override (`@name`)** — if the user's request contains an `@<name>` token (e.g. `@work save this`, `query @personal about X`), resolve `~/.beeweave/config.<name>` directly and use the full config from that file, including `BEEWEAVE_VAULT_PATH`, `BEEWEAVE_WORKBENCH_PATH`, QMD settings, and tool-specific paths. This **overrides** both the CWD `.env` walk-up and the default `~/.beeweave/config`, and applies to **that invocation only** — never run `ln -sf` or otherwise change `~/.beeweave/config` for an `@name` request. If `~/.beeweave/config.<name>` doesn't exist, tell the user it doesn't exist and list the available profiles (the `beeweave-switch` **List** logic), then stop — do **not** silently fall back to the default. The `@name` is a routing directive, not content: strip it out before treating the rest of the request as the actual instruction or page text.
...
`@name` is a **per-invocation profile override** — it targets one complete BeeWeave profile for one request. The default profile remains `~/.beeweave/config`; named profiles are selected explicitly with `@name` rather than activated by symlink. To intentionally make a named profile the default, use `bwe profile set-default <name>` or `/beeweave-switch set-default <name>`, which copies `config.<name>` to `config` after backing up the previous default.
...
find_config() {
  ...
  while [[ "$dir" != "$HOME" && "$dir" != "/" ]]; do
    [[ -f "$dir/.env" ]] && grep -q "BEEWEAVE_VAULT_PATH" "$dir/.env" && { echo "$dir/.env"; return; }
    dir="$(dirname "$dir")"
  done
  [[ -f "$HOME/.beeweave/config" ]] && { echo "$HOME/.beeweave/config"; return; }
  echo ""
}
```
...
Skills that write runtime state (e.g. `beeweave-daily-update`) must scope that state to the resolved vault, not to a global path. Use:

```
VAULT_ID=$(echo "$BEEWEAVE_VAULT_PATH" | md5sum 2>/dev/null || md5 -q - <<< "$BEEWEAVE_VAULT_PATH" | cut -c1-8)
...
- `BEEWEAVE_LINK_FORMAT` — Internal link syntax: `wikilink` (default) or `markdown`
- `WIKI_TOKEN_WARN_THRESHOLD` — Emit a warning in `beeweave-status` when the full-wiki token estimate exceeds this value (default: `100000`). Set to `0` to disable. See `beeweave-status` for the token footprint report.
- `WIKI_STAGED_WRITES` — When `true`, all LLM-written pages go to `_staging/<category>/` for human review before promotion. See `beeweave-setup` and `beeweave-stage-commit` for details.
...
Use `beeweave-status` to see the delta and get a recommendation. Use `beeweave-rebuild` for archive/rebuild/restore operations.

## Reference

For details on specific operations, see the companion skills:
- **beeweave-status** — Audit what's ingested, compute delta, recommend append vs rebuild
- **beeweave-rebuild** — Archive current wiki, rebuild from scratch, or restore from archive
- **beeweave-ingest** — Distill source documents into wiki pages and raw text/chat/log data
- **beeweave-claude-ingest** — Ingest Claude conversation history
- **beeweave-codex-ingest** — Ingest Codex CLI session history
- **beeweave-query** — Answer questions against the wiki
- **beeweave-lint** — Audit and maintain wiki health
- **beeweave-setup** — Initialize a new vault
