...
description: >
  ...
  "/beeweave-import", or wants to transfer pages from one vault to another using the output of beeweave-export.
---

# Wiki Import — Reconstruct Pages from an Export

You are importing a vault's knowledge into the current vault from one of two sources produced by `beeweave-export`:

- **`graph.json`** — the graph skeleton. Reconstructs page **stubs** (frontmatter, typed relationships, a `## Related` link list — no body). Lossy.
- **OKF bundle** (a `beeweave-export/okf/` directory) — the actual markdown files. Reconstructs **full pages** with their real bodies. Lossless. Use this for true vault-to-vault transfer.
...
2. Read `$BEEWEAVE_VAULT_PATH/AGENTS.md` if it exists — apply any owner-specific conventions.

## Step 1: Locate and Detect Source Type

**Find the import source:**
- If the user provided a path argument, use it directly.
- Otherwise auto-detect, in order: `./beeweave-export/okf/` (a directory) → `./beeweave-export/graph.json` (a file).
...
3. **Reverse-map frontmatter** (the inverse of the canonical mapping table in `beeweave-export` Step 3.5):
   - `title` ← `title`.
   - `category` ← the preserved `category` extension key if present; **else** lower-case the directory prefix of the concept id (`concepts/…` → `concepts`); **else** derive from `type` (`Concept`→`concepts`, `Entity`→`entities`, `Skill`→`skills`, `Reference`→`references`, `Synthesis`→`synthesis`, `Project`→`projects`).
   - `tags` ← `tags`.
   - `summary` ← `description`.
...
4. **Reverse-transform body links** — markdown links that point at `.md` paths become wikilinks (this restores both real cross-links and forward-references the exporter preserved per `beeweave-export` Step 3.5):
   ...
   - When `BEEWEAVE_LINK_FORMAT=markdown` is set in config, **keep** markdown links (just rewrite the path to be vault-relative); do not convert to wikilinks.
...
"<absolute path to source>": {
  ...
  "source_type": "beeweave-export",
  "pages_created": ["list/of/created/pages.md"],
  "pages_updated": ["list/of/merged/pages.md"]
}
```

Set `source_type` to `"beeweave-export"` for a graph.json import or `"okf-bundle"` for an OKF bundle import. Key the entry by the absolute path of the source (the `graph.json` file or the bundle directory).
...
```
Wiki import complete → $BEEWEAVE_VAULT_PATH
