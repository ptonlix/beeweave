---
name: beeweave-vault-skill-factory
description: >
  ...
  Use this skill when the user says "/beeweave-vault-skill-factory", "make a skill from my wiki", "turn these
...
and (only if they choose) install. This is the inverse of `beeweave-capture`: capture turns a
...
1. **Resolve config** (Config Resolution Protocol in `beeweave-core/SKILL.md`): get `BEEWEAVE_VAULT_PATH`,
   `BEEWEAVE_REPO`, `BEEWEAVE_LINK_FORMAT`, the QMD vars, and:
   - `SKILL_FACTORY_OUTPUT_DIR` — where generated skills land. Default:
     `$BEEWEAVE_VAULT_PATH/_generated-skills` (a vault-level, underscore-prefixed *excluded* dir —
     like `_meta`/`_staging`/`_sources`, NOT the `skills/` knowledge category). This co-locates
...
- **`description`** — the trigger. Write it "pushy" (per `beeweave-skill-creator`): state **when** to use it
...
## Step 4: Optionally lean on beeweave-skill-creator

`beeweave-skill-creator` ships reusable scripts (`$BEEWEAVE_REPO/.skills/beeweave-skill-creator/scripts/`):
