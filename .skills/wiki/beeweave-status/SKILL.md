...
⚠️  Full wiki exceeds 100K tokens. Consider:
  - Demoting peripheral pages (promote tier suggestions from beeweave-status insights mode)
  - Running /beeweave-lint --consolidate to merge near-duplicates
  - Using beeweave-query fast mode for most queries
```
...
4. **Synthesis opportunities** — check `hot.md` for any recent `/beeweave-synthesize` run summary. If the last synthesis run reported N opportunities, surface that count. If no synthesis has been run recently (not in `hot.md` or `log.md` within last 14 days), flag it as "synthesis scan overdue".

5. **Source delta** — from Step 2: count of new + modified sources ready to ingest.

6. **Lint issues** — check `log.md` for a recent `/beeweave-lint` run (within last 30 days). If a recent run recorded broken links or missing frontmatter, surface the count. If no lint run appears in the log, flag "lint not run recently".
...
| 1 | Inbox files waiting | Any files present in `workbench/inbox/captures/` or `workbench/inbox/web/` |
...
0. 📋  6 staged pages waiting for review (oldest: 3 days ago)
   ...
   run: /beeweave-stage-commit

1. 📥  Ingest 3 files waiting in workbench/inbox/
...
2. 🔄  Refresh 2 stale core pages (not updated in 90+ days)
   ...
   run: open these pages and re-run /beeweave-update

3. 🔗  Link 7 orphan pages  →  run: /beeweave-cross-linker
   Disconnected: [[Redis Caching]], [[JWT Tokens]], +5 more

4. 🧩  2 synthesis opportunities identified  →  run: /beeweave-synthesize
   [[Redis Caching]] × [[Session Management]] (co-occur in 8 pages)

5. ✅  4 sources modified since last ingest  →  run: /beeweave-ingest (append mode)

6. 🩺  Lint not run in 30+ days — run: /beeweave-lint
```

**Empty state:** If all categories have nothing to report (no staged files, no inbox files, no orphans, no stale pages, no synthesis opportunities, no new sources, no lint issues), output instead:

```markdown
## What to Do Next

✅  Wiki is healthy — nothing urgent.
    All sources up to date · no orphans · no stale core pages · no inbox files pending · no staged writes
```

**Overflow:** If more than 6 items would be shown, add a footer line: `_(N more items available — run /beeweave-status --full to see all)_`. The `--full` flag is not yet implemented; this is forward-looking copy that sets expectations.
...
Where the delta report tells the user what's pending, insights mode tells them what they've already built and where the interesting structure lives. Complements `beeweave-lint` (which finds *problems*) by surfacing *interesting structure*.

### What to compute

**First, run the graph analyser.** This replaces manual wikilink parsing — one command produces all the raw data you need:

Use `$BEEWEAVE_CLI` when it is set. Otherwise use `bwe`; in a source checkout where the package entrypoint is not installed, use `python3 -m beeweave`.

```bash
${BEEWEAVE_CLI:-bwe} graph-analyse "$BEEWEAVE_VAULT_PATH" --pretty
```
...
- `dead_ends` — pages with zero outgoing links. Use for orphan-adjacent and beeweave-cross-linker suggestions.
- `isolated` — pages with zero links in either direction. Use for stubs/orphan reporting.
- `stats` — total pages, edges, communities.

**Fallback** (if `bwe` is not installed): glob all `.md` pages, extract every `[[wikilink]]`, and build `incoming`, `outgoing`, and `tags` maps manually.
...
1. **Anchor pages (top hubs).** Pages with the most incoming links — the load-bearing concepts.
   ...
   - Pages with high incoming but zero outgoing are sink hubs — flag as beeweave-cross-linker candidates
...
3. **Tag cluster cohesion.** For each tag with ≥ 5 pages, score how tightly the pages within it are interconnected:
   ...
   - **Fragmented clusters** (cohesion < 0.15, n ≥ 5): these pages share a topic but aren't woven together. Surface them as beeweave-cross-linker targets.
...
5. **Orphan-adjacent suggestions.** Pages linked from a top-10 hub but with zero outgoing links of their own. Dead-ends in high-traffic areas — prime beeweave-cross-linker candidates.
...
| [[entities/andrej-karpathy]] | 17 | 0 | sink hub — beeweave-cross-linker candidate |
...
### Most fragmented (beeweave-cross-linker targets)
- **#systems** — 7 pages, cohesion 0.06 ⚠️ run beeweave-cross-linker on this tag
...
- After a fresh `beeweave-rebuild` — wait until at least one ingest has happened.
...
- The actual ingest work is done by the ingest skills (`beeweave-ingest`, `beeweave-claude-ingest`, `beeweave-codex-ingest`)
