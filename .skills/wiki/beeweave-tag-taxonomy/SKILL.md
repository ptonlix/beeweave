...
**`hot.md`** — Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from the template in `beeweave-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Tag audit: normalized 14 tags across 28 pages; 2 new canonical tags added." Keep the last 3 operations. Update `updated` timestamp.
