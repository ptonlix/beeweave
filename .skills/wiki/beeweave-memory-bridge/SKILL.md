---
name: beeweave-memory-bridge
description: >
  Browse and compare wiki knowledge by which AI tool originally produced it. Use this skill when the user
  says "/beeweave-memory-bridge", "browse codex memory", "what did codex know about X", "show me claude knowledge",
...
2. Read `$BEEWEAVE_VAULT_PATH/.manifest.json` — this is the source-of-truth for what tool produced what.
3. Read `$BEEWEAVE_VAULT_PATH/index.md` for page titles and one-line descriptions.
...
| Invocation | Mode |
|---|---|
| `/beeweave-memory-bridge <tool>` | **Browse** — list all wiki pages sourced from `<tool>` |
| `/beeweave-memory-bridge <tool> "<topic>"` | **Search** — pages from `<tool>` that mention `<topic>` |
| `/beeweave-memory-bridge diff` | **Diff** — pages unique to each tool; overlap; blind spots |
| `/beeweave-memory-bridge diff <tool-a> <tool-b>` | **Diff** — compare two specific tools |
| `/beeweave-memory-bridge map` | **Map** — full origin matrix: every page × every tool that touched it |

Recognized tool names: `claude`, `codex`, `hermes`, `openclaw`, `copilot`, `pi`, `manual` (hand-written), `ingest` (beeweave-ingest documents).
...
## Step 3: Spawn beeweave-impl-validator (if available)

After generating output, if the `beeweave-impl-validator` skill is available in the current environment, spawn it as a subagent:
...
- Use `[[wikilinks]]` for page references (or standard Markdown links if `BEEWEAVE_LINK_FORMAT=markdown` is set).
- In diff mode, call out the most *surprising* asymmetry explicitly — that's the insight the user came for.
- If `.manifest.json` is empty or missing, say so clearly and suggest running `/beeweave-history-ingest` first.
