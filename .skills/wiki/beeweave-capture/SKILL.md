---
name: beeweave-capture
description: >
  Save the current conversation as a permanent, structured wiki note. Use this skill when the user
  says "save this", "/beeweave-capture", "capture this", "file this conversation", "preserve this",
  ...
  it in the correct vault category. Also supports a fast QUICK MODE (`/beeweave-capture --quick`, "quick
...
- **Quick mode (`--quick`)** — zero-friction staging: drop findings to `workbench/inbox/captures/` in under 60 seconds with no manifest/index/log/QMD writes. Used for mid-session capture and by the session-end Stop hook. See below, then stop — do **not** run the full-mode steps.

## Quick Mode (`--quick`)

Trigger when invoked as `/beeweave-capture --quick`, by "quick capture" / "capture this finding" / "save this bug fix" / "save this gotcha" / "drop this to inbox" / "quick save to wiki", or automatically by the session-end Stop hook.
...
4. **Cluster by topic** — one inbox capture file per topic cluster, not per finding. Name each as a kebab-case slug (e.g. `swift-actor-reentrancy`, `nextjs-hydration-mismatch`).
...
7. **Confirm** — list staged files and tell the user to run `/beeweave-ingest` to promote them:
   ```
   Staged to workbench/inbox/captures/:
     workbench/inbox/captures/2026-05-27-swift-actor-reentrancy.md   — "Actor reentrancy causes deadlock in async forEach"
   Run /beeweave-ingest to promote these to full wiki pages.
   ```
   Quick mode deliberately does **not** write the manifest, `index.md`, `log.md`, `hot.md`, or refresh QMD — promotion via `/beeweave-ingest` handles all of that. **Stop here; do not run the full-mode steps below.**
...
1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and `BEEWEAVE_LINK_FORMAT` (default: `wikilink`).
2. Read `$BEEWEAVE_VAULT_PATH/index.md` to understand existing wiki content (avoid duplicates)
3. Read `$BEEWEAVE_VAULT_PATH/hot.md` if it exists — it gives context on recent activity

When writing internal links in Step 5, apply the link format from `beeweave-core/SKILL.md` (Link Format section) using the `BEEWEAVE_LINK_FORMAT` value.
...
Apply provenance markers per `beeweave-core`:
...
title: >-
  <Title>
category: <synthesis|concepts|references|skills>
