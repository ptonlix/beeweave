# Inbox Capture Format Reference

Full specification for files written by `beeweave-capture --quick` under
`workbench/inbox/captures/`. These files are designed to be promoted by
`/beeweave-ingest` and then moved to `workbench/inbox/archived/`.
...
Apply provenance markers inline per the `beeweave-core` convention:
