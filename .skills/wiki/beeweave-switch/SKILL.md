...
Each vault is a complete config file at `~/.beeweave/config.<name>`. The active vault is
whichever file `~/.beeweave/config` symlinks to. Switching vaults means re-pointing that symlink.
...
1. Verify `~/.beeweave/config.<name>` exists. If not, tell the user the vault doesn't exist and list what's available (run **List**).
2. Run:
   ```bash
   ln -sf ~/.beeweave/config.<name> ~/.beeweave/config
   ```
3. Read `BEEWEAVE_VAULT_PATH` from the newly active config.
...
- If no name given, read `~/.beeweave/config` (the active vault).
...
6. Update the top comment line to describe the new vault (e.g., `# BeeWeave — <name> vault`).
7. Confirm:
   ```
