...
description: >
  ...
  invokes /beeweave-claude, /beeweave-codex, /beeweave-hermes, /beeweave-openclaw, /beeweave-copilot, /beeweave-pi — with or without a
  search topic. Different from beeweave-history-ingest (which bulk-ingests everything new): this skill finds
...
|---|---|---|
| `/beeweave-claude [query]` | Claude Code history | `/beeweave-claude "how did I set up auth middleware"` |
| `/beeweave-codex [query]` | Codex CLI history | `/beeweave-codex "rust ownership patterns"` |
| `/beeweave-hermes [query]` | Hermes agent history | `/beeweave-hermes "memory architecture"` |
| `/beeweave-openclaw [query]` | OpenClaw history | `/beeweave-openclaw "project planning approach"` |
| `/beeweave-copilot [query]` | Copilot chat history | `/beeweave-copilot "test strategy for API routes"` |
| `/beeweave-pi [query]` | Pi agent history | `/beeweave-pi "how did I refactor the auth module"` |

If no query is given, default to **recent sessions mode**: ingest the last 5 unprocessed sessions from that agent and return a summary of what was found. This is equivalent to a focused `beeweave-history-ingest` for that agent only.
...
2. Read `$BEEWEAVE_VAULT_PATH/.manifest.json` → know what's already ingested.
3. Read `$BEEWEAVE_VAULT_PATH/hot.md` if it exists → warm context on recent wiki activity.
...
2. **Determine category** using standard rules (from `beeweave-core/SKILL.md`):
...
3. **Write or update the page** with required frontmatter:
   ...
   ---
   ```
   Set `sources` with the agent prefix so `beeweave-memory-bridge` can find it later.
...
→ `/beeweave-claude "X"` — finds Claude sessions about X, ingests them, returns the answer

**"I solved a bug in Hermes last week. I need that context now in Claude Code."**
→ `/beeweave-hermes "bug description"` — surfaces and ingests the Hermes session

**"What are all the approaches I've tried for X across all my tools?"**
→ Run `/beeweave-claude "X"`, `/beeweave-codex "X"`, `/beeweave-hermes "X"` in sequence — each ingests its slice, the wiki accumulates the cross-agent picture, then `/beeweave-memory-bridge diff` shows what each tool uniquely contributed

**No query — just "catch me up on recent Codex work"**
→ `/beeweave-codex` — ingests last 5 Codex sessions and returns a summary

**"I'm on Claude Code. What did I figure out about X in Pi?"**
→ `/beeweave-pi "X"` — finds Pi sessions about X, ingests them, returns the answer

**No query — just "catch me up on recent Pi work"**
→ `/beeweave-pi` — ingests last 5 Pi sessions and returns a summary
