...
description: >
  ...
  it actually modifies pages to add links, unlike beeweave-lint which just reports issues.
---
...
When inserting links in Step 4, apply the link format from `beeweave-core/SKILL.md` (Link Format section) using the `BEEWEAVE_LINK_FORMAT` value. When `BEEWEAVE_LINK_FORMAT=markdown`, compute the relative `.md` path from the **file being edited** to the target page.
...
Always use wikilink format (`[[path/to/page]]`) for `target` values in the `relationships:` YAML block — regardless of `BEEWEAVE_LINK_FORMAT`. The `BEEWEAVE_LINK_FORMAT` setting controls body content; frontmatter properties always use wikilink syntax so that `beeweave-export` can reliably parse them.

Only add entries for links added in this beeweave-cross-linker run — do not touch typed entries that were already present.
...
**`hot.md`** — Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from the template in `beeweave-ingest` if missing). Update **Recent Activity** with a one-line summary of what was linked — e.g. "Cross-linked 23 mentions across 12 pages; 2 orphans remain." Keep the last 3 operations. Update `updated` timestamp.
