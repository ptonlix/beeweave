---
name: beeweave-stage-commit
description: >
  Review and promote staged wiki pages to their final locations. Use when WIKI_STAGED_WRITES=true
  and the user says "/beeweave-stage-commit", "review staged pages", "commit staged writes",
  ...
  their final wiki locations. Rejected files are moved to `workbench/inbox/rejected/`
  for manual editing.
---
...
1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md`. This gives `BEEWEAVE_VAULT_PATH` and `WIKI_STAGED_WRITES`.
...
❌  Rejected (M):
  skills/fine-tuning-llms.md          → moved to workbench/inbox/rejected/rejected-skills-fine-tuning-llms.md
