...
1. Glob `$VAULT_PATH/**/*.md` excluding `_archives/`, `_staging/`, `_meta/`, legacy `_raw/`, `.obsidian/`, `node_modules/`, `index.md`, `log.md`, `_insights.md`.
...
Use the six vault top-level folders in this fixed order so colors are stable across runs:
...
- The palette here matches `beeweave-export`'s `graph.html` community colors, so the Obsidian graph and the exported visualization look consistent.
