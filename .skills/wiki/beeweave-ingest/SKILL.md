...
description: >
  ...
  "save this to my wiki". Also triggers when the user drops a file, or for inbox mode: "process my
  drafts", "promote my inbox captures", or any reference to `workbench/inbox/`. This is the
  general catch-all ingest skill for any document, text, or URL source not covered by a more
  specific ingest skill (beeweave-claude-ingest, etc.).
---
...
Use `$BEEWEAVE_CLI` for helper commands when it is set. Otherwise use `bwe`; in a source checkout where the package entrypoint is not installed, use `python3 -m beeweave`.

```bash
${BEEWEAVE_CLI:-bwe} cache-check "$BEEWEAVE_VAULT_PATH" <source1> [source2 ...]
```
...
```bash
${BEEWEAVE_CLI:-bwe} cache-update "$BEEWEAVE_VAULT_PATH" <source> --pages <page1> [page2 ...]
```

**Fallback** (if `bwe` is not installed): compute hashes manually with `sha256sum -- "<file>"` (Linux) or `shasum -a 256 -- "<file>"` (macOS) and compare against `content_hash` in `.manifest.json`. If the entry has no `content_hash`, fall back to mtime comparison.
...
- After a `beeweave-rebuild` has cleared the vault

### Inbox Mode
Process pending inputs from `workbench/inbox/`. Use when:
- The user says "process my drafts", "promote my inbox captures", or drops files into `workbench/inbox/`
...
**Source inheritance:** The inbox path is a staging artifact — never use it as the `sources:` value on the promoted page. Derive the source entry from the inbox file's own frontmatter instead:

- If the file has both `capture_source` and `sources:` fields, synthesize a combined entry:
  `"agent:<capture_source> <sources-value>"` — e.g. `"agent:claude-session beeweave session (2026-05-29)"`
...
**GUARD: Only run this step when the source is a directory with more than 20 files.** For single files, small folders, or inbox mode, skip directly to Step 1.

When the source is a large directory of docs, plan the parallel dispatch first:

```bash
${BEEWEAVE_CLI:-bwe} batch-plan "$BEEWEAVE_VAULT_PATH" <source-dir> --pretty
```
...
4. **If `batch_count > 1`** — dispatch each batch as a **parallel subagent** (multiple Agent tool calls in a single message). Each subagent receives a message like:
   ```
   Ingest these files into the wiki at $BEEWEAVE_VAULT_PATH using beeweave-ingest Step 1 onward:
   <list of file paths from this batch>
   Skip batch-plan — these files are already partitioned.
   ```
   Wait for all subagents to complete, then run `/beeweave-cross-linker` once to wire cross-references across all batches.

**Fallback** (if `bwe` is not installed): process files sequentially in groups of 15.
...
5. **Write the page with the Paper Deep-Dive Template** (`beeweave-core/SKILL.md`) into `references/`, in addition to the distilled concept/entity cross-links. This is the deliberate exception to "aim for 10–15 small pages" (Step 4) — a paper earns one rich, self-contained page.
...
```bash
${BEEWEAVE_CLI:-bwe} ast-extract <path> --pretty
```
...
If `bwe` is not installed or the command fails, skip this step and proceed to Step 2 as normal — it is an optimisation, not a requirement.
...
- **Relationships** between concepts — note the *type* when the source text makes it clear. Use the allowed types from `beeweave-core/SKILL.md` (Typed Relationships section): `extends`, `implements`, `contradicts`, `derived_from`, `uses`, `replaces`, `related_to`. Record: source page, target page, inferred type.
...
- Does this page already exist? (Check `index.md` and use Glob to search `BEEWEAVE_VAULT_PATH`)
...
**Apply tier-aware filtering to existing pages** (see `beeweave-core/SKILL.md`, Importance Tiering section):
...
- Use the page template from the beeweave-core skill (frontmatter + sections). **For academic papers landing in `references/`, use the Paper Deep-Dive Template** from `beeweave-core/SKILL.md` instead of the generic one (see *Academic papers* in Step 1).
- Place in the correct category directory
- Add `[[wikilinks]]` to at least 2-3 existing pages
- Include the source in the `sources` frontmatter field. In inbox mode: derive from `capture_source` + `sources` frontmatter of the inbox file — never use the inbox path itself (see Inbox Mode section)
...
**Populate `relationships:` when context is clear** — if Step 2 identified typed relationships between this page and another, add a `relationships:` block to the frontmatter (defined in `beeweave-core/SKILL.md`, Typed Relationships section). Only add entries where the source text makes the direction and type unambiguous. When in doubt, use `related_to` or omit the block. Example:
...
**Apply provenance markers** per the convention in `beeweave-core` (Provenance Markers section):
...
**`hot.md`** — Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from template below if missing). Rewrite the **Recent Activity** section to reflect what you just ingested — keep it to the last 3 operations max. Update **Key Takeaways** and **Active Threads** if the content materially shifted them. Update the `updated` timestamp.
...
- [ ] `relationships:` block is present on pages where source text made typed connections clear; all entries use an allowed type from `beeweave-core/SKILL.md`
