...
Reference for the `beeweave-ingest` skill when the source is a **web URL** rather than a local file.
...
main `beeweave-ingest/SKILL.md` — follow those; this file only covers the URL-specific mechanics.
...
4. **No project context** — if the current directory IS the beeweave repo itself, or if detection produces a name that matches the wiki vault directory, treat it as "no project context" and fall back to `misc/`.

**Normalise the project name:** lowercase, replace spaces and underscores with `-`, strip leading dots.

Once you have a candidate name, check whether `$BEEWEAVE_VAULT_PATH/projects/<project-name>/` exists:
...
- If in project mode: grep `$BEEWEAVE_VAULT_PATH/projects/<project-name>/` for the URL string
- If in misc mode: grep `$BEEWEAVE_VAULT_PATH/misc/` for the URL string
...
Target: `$BEEWEAVE_VAULT_PATH/projects/<project-name>/references/<slug>.md`
...
Target: `$BEEWEAVE_VAULT_PATH/misc/<slug>.md`
...
> ⚡ Strong affinity detected: this page has **3+ connections** to `<project-name>`. Run the `beeweave-cross-linker` skill to recompute affinity and then consider promoting this page to `projects/<project-name>/references/`.
