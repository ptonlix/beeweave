...
You are distilling knowledge from the current project into the user's Obsidian wiki. This skill works from any project directory, not just the beeweave repo.

## Before You Start

1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH`, `BEEWEAVE_REPO`, `BEEWEAVE_LINK_FORMAT` (`wikilink` default or `markdown`), and optional QMD settings such as `QMD_WIKI_COLLECTION`. Works from any project directory.
3. Read `$BEEWEAVE_VAULT_PATH/.manifest.json` to check if this project has been synced before.
4. Read `$BEEWEAVE_VAULT_PATH/index.md` to know what the wiki already contains.

When writing internal links in Steps 4–5, apply the link format from `beeweave-core/SKILL.md` (Link Format section) using the `BEEWEAVE_LINK_FORMAT` value.
...
**Write a `summary:` frontmatter field** on every new/updated page (1–2 sentences, ≤200 chars), using `>-` folded style. For project sync, a good summary answers "what does this page tell me about the project I wouldn't guess from its title?" This field powers cheap retrieval by `beeweave-query`.

**Apply provenance markers** per `beeweave-core` (Provenance Markers section). For project sync specifically:
...
Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from the template in `beeweave-ingest` if missing). Rewrite **Recent Activity** with what was just synced — last 3 operations max. Update **Active Threads** if this project is an ongoing focus. Update **Key Takeaways** with the most important architectural insight or decision surfaced during this sync. Update `updated` timestamp.

Write conceptually: "Synced beeweave — added beeweave-capture and beeweave-research skills, core new capabilities are autonomous web research and conversation capture."
