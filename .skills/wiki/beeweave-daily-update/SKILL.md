---
name: beeweave-daily-update
description: >
  Run the daily wiki maintenance cycle: check all source freshness, update the index, and regenerate hot.md.
  Use this skill when the user says "/beeweave-daily-update", "run the daily update", "update everything", "morning sync",
...
**Step 5: Spawn beeweave-impl-validator**

After the cycle, spawn `beeweave-impl-validator` as a subagent:

```
beeweave-impl-validator check:
  goal: "Daily wiki maintenance — index reconciled, hot.md refreshed, state file written"
  artifacts:
    - $BEEWEAVE_VAULT_PATH/index.md
    - $BEEWEAVE_VAULT_PATH/hot.md
...
Stale sources (run to sync):
  /beeweave-history-ingest claude   — N sessions since last ingest
  /beeweave-history-ingest codex    — N sessions since last ingest
```
...
Check that `$BEEWEAVE_REPO/scripts/beeweave-daily-update.sh` exists and is executable. If not, point the user to it.

**Step 2: Install launchd plist**

```bash
# Replace placeholder in plist
sed "s|BEEWEAVE_REPO|$BEEWEAVE_REPO|g" \
  "$BEEWEAVE_REPO/scripts/com.beeweave.beeweave-daily-update.plist" \
  > "$HOME/Library/LaunchAgents/com.beeweave.beeweave-daily-update.plist"

# Load it
launchctl load "$HOME/Library/LaunchAgents/com.beeweave.beeweave-daily-update.plist"
```
...
Check if `beeweave-notify.sh` is already sourced in that rc file. If not, append:

```bash
echo "" >> "$RC_FILE"
echo "# beeweave terminal notification" >> "$RC_FILE"
echo "source $BEEWEAVE_REPO/scripts/beeweave-notify.sh" >> "$RC_FILE"
```

For Fish shell, source syntax is different — provide the manual instruction:
```fish
# Add to ~/.config/fish/config.fish:
bass source $BEEWEAVE_REPO/scripts/beeweave-notify.sh
...
```bash
bash "$BEEWEAVE_REPO/scripts/beeweave-daily-update.sh"
```
...
- State is stored in `~/.beeweave/state/<vault-id>/` — supports multiple vaults independently
- They can run `/beeweave-daily-update` anytime to force a sync
- Logs go to `/tmp/beeweave-daily.log`
