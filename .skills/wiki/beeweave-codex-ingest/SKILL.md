...
This skill can be invoked directly or via the `beeweave-history-ingest` router (`/beeweave-history-ingest codex`).

## Before You Start

1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and `CODEX_HISTORY_PATH` (defaults to `~/.codex`)
...
**`hot.md`** — Read `$BEEWEAVE_VAULT_PATH/hot.md` (create from the template in `beeweave-ingest` if missing). Update **Recent Activity** with a one-line summary — e.g. "Ingested 12 Codex sessions; surfaced recurring patterns in CLI tooling and shell scripting." Keep the last 3 operations. Update `updated` timestamp.
