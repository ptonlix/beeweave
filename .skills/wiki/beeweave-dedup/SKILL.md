...
description: >
  ...
  for the same thing". Distinct from beeweave-lint (which checks structure) and beeweave-cross-linker (which adds
...
**Follow the Retrieval Primitives table in `beeweave-core/SKILL.md`.** The candidate-detection pass uses only frontmatter and titles (cheap). Only open full page bodies for confirmed candidate pairs.
...
Glob all `.md` files in the vault (excluding `_archives/`, `_staging/`, `_meta/`, legacy `_raw/`, `.obsidian/`, `index.md`, `log.md`, `hot.md`, `_insights.md`, and any file that contains `redirects_to:` in its frontmatter — those are already merged redirect stubs).
...
- **`base_confidence`** — recompute using the union of sources and the formula from `beeweave-core/SKILL.md`
- **`updated`** — set to now
...
- If `BEEWEAVE_LINK_FORMAT=markdown`: `[text](../path/to/secondary.md)` → `[text](../path/to/canonical.md)`
...
- **`beeweave-export`** — skip pages with `redirects_to:` in frontmatter; they are not content nodes
- **`beeweave-query`** — if a search hits a redirect stub, follow `redirects_to:` and read the canonical page instead
- **`beeweave-lint`** — validate that every `redirects_to:` wikilink resolves to an existing, non-stub page (a redirect chain — stub pointing to stub — is an error)
- **`beeweave-cross-linker`** — treat redirect stubs as non-targets; never add a new `[[wikilink]]` pointing at a stub page
...
- **Run `beeweave-cross-linker` after dedup.** The redirect stubs leave the graph in a slightly inconsistent state. Cross-linker will tighten it up.
