...
description: >
  ...
  Use when the user says "/beeweave-context-pack", "make a context pack", "give me a context slice for X",
  "pack the wiki for my agent", or "bounded context for Y". Different from beeweave-query (which answers a
  question) — this produces reusable input material for a downstream task.
---

# Wiki Context Pack — Bounded Token Retrieval

You are producing a focused, token-bounded context pack from the wiki. Unlike `beeweave-query` (which answers a question), this skill packages the most relevant wiki knowledge into a single markdown block that a downstream agent, skill, or user can consume directly.

## Before You Start

1. **Resolve config** — follow the Config Resolution Protocol in `beeweave-core/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.beeweave/config` → prompt setup). This gives `BEEWEAVE_VAULT_PATH` and any QMD variables.
2. Read `$BEEWEAVE_VAULT_PATH/hot.md` if it exists — gives instant context on recent activity.
3. Read `$BEEWEAVE_VAULT_PATH/index.md` — the full page inventory.

## Invocation Forms

```
/beeweave-context-pack "transformer attention mechanism" --budget 16000
/beeweave-context-pack "my-project architecture decisions" --budget 8000
/beeweave-context-pack --recent --budget 4000   # recent activity pack from hot.md
/beeweave-context-pack "authentication patterns"          # default budget: 8000 tokens
```
...
Within the candidate set, sort by relevance score, then apply tier ordering within each score bucket (see `beeweave-core/SKILL.md`, Importance Tiering section):
...
```
# Context Pack: <topic>
No relevant pages found. Consider running /beeweave-ingest to add sources about this topic.
```

### Step 6: Log

Append to `$BEEWEAVE_VAULT_PATH/log.md`:
```
...
- **Feed into `/beeweave-research`** — pass the pack as context to avoid re-discovering known facts
- **Pass to `/beeweave-synthesize`** — scoped input for a specific synthesis task
...
- The `4 chars/token` heuristic matches `beeweave-status`'s token footprint estimate — consistent across skills
- The pack is a snapshot; it is not written to the vault. Re-run to refresh.
- For very large budgets (> 50K tokens), warn the user: "This pack is large. Consider narrowing your topic or using beeweave-query for a targeted answer instead."
